...
def encode_string(string_to_encode: str, current_date: str, transaction_id: str, otp: str, nonce: bytes) -> dict:
    """
    ...
    """
    encryptor = Encryptor(current_date=current_date, transaction_id=transaction_id, otp=otp, nonce=nonce)

    return encryptor.encode_string(string_to_encode=string_to_encode)
...
class Encryptor:
    ...
    def __init__(self, current_date, transaction_id, otp, nonce):
        ...
        self.__otp = otp
        self.__encryption_key = EncryptorUtils.generate_encryption_key_from_fields(current_date=self.__current_date,
                                                                                   transaction_id=self.__transaction_id,
                                                                                   otp=self.__otp)
        self.__nonce = nonce

    def encode_string(self, string_to_encode: str) -> dict:
        """
        ...
        ciphertext, tag, nonce = self._encrypt_string(string_to_encrypt=string_to_encode)

        output = EncryptorUtils.output_as_dict([ciphertext, tag, nonce])
        return output

    def _encrypt_string(self, string_to_encrypt: str) -> (str, str, str):
        """
...
def _convert_string_to_hex(string_to_convert: str) -> str:
    ...
    """
    input_string_as_hex_values = EncryptorUtils.convert_string_to_hex_string(string_to_convert=string_to_convert)
