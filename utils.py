...
class EncryptorUtils:
    @staticmethod
    def generate_encryption_key_from_fields(current_date: str, transaction_id: str, otp: str) -> str:
        """
        ...
        hex_current_date = EncryptorUtils._convert_integer_string_to_hex_value(integer_string_to_convert=day_and_month)
        padded_hex_current_date = EncryptorUtils.pad_string_to_length_with_zeroes(string_to_pad=hex_current_date,
                                                                                  padded_length=KEY_DATE_HEX_LENGTH)

        padded_transaction_id = EncryptorUtils.pad_string_to_length_with_zeroes(
            string_to_pad=transaction_id, padded_length=KEY_TRANSACTION_ID_HEX_LENGTH)

        hex_otp = EncryptorUtils._convert_integer_string_to_hex_value(integer_string_to_convert=otp)
        padded_hex_otp = EncryptorUtils.pad_string_to_length_with_zeroes(string_to_pad=hex_otp,
                                                                         padded_length=KEY_OTP_HEX_LENGTH)

        encryption_key = "".join([padded_hex_current_date, padded_transaction_id, padded_hex_otp])
